...
def plackett_luce_log_prob(logits, actions):
    ...
    for i in range(K):
        # 创建新掩码（非原地）
        current_mask = mask.clone()  # 显式复制
        if i > 0:
            current_mask = current_mask.scatter(1, actions[:, i - 1:i], True)  # 非原地scatter

        # 应用掩码
        masked_logits = logits.masked_fill(current_mask, -1e6)  # 改用-1e6更稳定
        log_p = F.log_softmax(masked_logits, dim=1)
        log_probs.append(log_p.gather(1, actions[:, i:i + 1]))

        # 更新掩码（非原地）
        mask = mask.scatter(1, actions[:, i:i + 1], True)  # 或直接使用current_mask
...
class PPOAgent:
    ...
    def update(self, states, actions, log_probs_old, returns, advantages, step, loss,
        ...
        if step % 50 == 0:
            torch.save(self.actor.state_dict(), 'model_new_16.pth')
...
for episode in range(MAX_EPISODE):
    ...
    for T_s in range(0, 10):  # 2s
        ...
        while True:
            ...
            actions.append(action.cpu()[0].numpy())
            log_probs.append(log_prob.item())
            traffic1, traffic2, traffic3, state, reward, reward1, reward2, reward3, reward4, done = step(state,
                                                                                                         action.cpu()[
                                                                                                             0].numpy(),
                                                                                                         user_cell_index,
                                                                                                         channel_gain_matrix[
            ...
            # print(reward3s)            # print(traffic_sca[6])
            dones.append(done)
            values.append(value)
            if done:
                break
        ...
        agent.update(states, actions, log_probs, returns, advantages, episode, loss)
        # print(rewards)
    ...
    np.savetxt('reward_3e-5_16.txt', Reward)
    np.savetxt('reward1_3e-5_16.txt', Reward1)
    np.savetxt('reward2_3e-5_16.txt', Reward2)
    np.savetxt('reward3_3e-5_16.txt', Reward3)
    np.savetxt('reward4_3e-5_16.txt', Reward4)
