...
class PPOAgent:
    ...
    def update(self, states, actions, powers, log_probs_old, returns, advantages, step, loss,
        ...
        if step % 50 == 0:
            torch.save(self.actor.state_dict(), 'model_actor_2.5.pth')
            torch.save(self.critic.state_dict(), 'model_critic_2.5.pth')
...
def step(state, action, power, user_cell_index, channel_gain_matrix, user_lambda1, user_lambda2, user_lambda3,
    ...
    delay1 = np.divide(np.sum(traffic1[:, 1:30] * weights1[1:30], axis=1),
                       traffic_sums1, where=traffic_sums1 != 0, out=np.zeros_like(traffic_sums1))
    delay2 = np.divide(np.sum(traffic2[:, 1:10] * weights2[1:10], axis=1),
                       traffic_sums2, where=traffic_sums2 != 0, out=np.zeros_like(traffic_sums2))
    delay3 = np.divide(np.sum(traffic3[:, 1:100] * weights3[1:100], axis=1),
    ...
    r1 = np.sum(RR) / (np.sum(user_lambda1) + np.sum(user_lambda2) + np.sum(user_lambda3)) / (np.sum(P) / 250)
    ...
    return traffic1, traffic2, traffic3, next_state, reward, r1, r2, r3, r4, done
...
#agent.actor.load_state_dict(torch.load("model_actor_2.5.pth", map_location=torch.device('cpu')))

# 加载 Critic 网络参数
#agent.critic.load_state_dict(torch.load("model_critic_2.5.pth", map_location=torch.device('cpu')))
...
for episode in range(MAX_EPISODE):
    ...
    for T_s in range(0, 10):  # 2s
        ...
        while True:
            ...
            powers.append(power.cpu().numpy()[0])
            log_probs.append(float(log_prob))  # 保证是纯 float

            traffic1, traffic2, traffic3, state, reward, reward1, reward2, reward3, reward4, done = step(
                state,
                actions[-1],
                powers[-1],
                user_cell_index,
    ...
    np.savetxt('reward_3e-5_2.5_2.txt', Reward)
    np.savetxt('reward1_3e-5_2.5_2.txt', Reward1)
    np.savetxt('reward2_3e-5_2.5_2.txt', Reward2)
    np.savetxt('reward3_3e-5_2.5_2.txt', Reward3)
    np.savetxt('reward4_3e-5_2.5_2.txt', Reward4)
