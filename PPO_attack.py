...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim):
        ...
        self.power_allocation = nn.Sequential(
            nn.Linear(256, 64),
            nn.ReLU(),
            nn.Linear(64, 9),  # 增加一层隐层
            nn.Softplus()
        )
    ...
    def forward(self, state):
        ...
        alpha = self.power_allocation(enc) + 1e-6
        dist_con = Dirichlet(alpha)
        powers = dist_con.sample()
        log_prob_con = dist_con.log_prob(powers)

        total_log_prob = log_prob_dis + log_prob_con
        return actions.detach(), powers, total_log_prob

    def evaluate_actions(self, state, actions, powers):
        enc = self.net(state)  # [B, 128]
        ...
        total_log_prob = log_prob_dis + log_prob_con
        total_entropy = entropy + dist_con.entropy()
...
class PPOAgent:
    ...
    def update(self, states, actions, powers, log_probs_old, returns, advantages, step, loss,
        ...
        powers = torch.tensor(powers, dtype=torch.float32).to(device)
        log_probs_old = torch.tensor(log_probs_old, dtype=torch.float32).to(device)
        returns = torch.tensor(returns, dtype=torch.float32).to(device)
        advantages = torch.tensor(advantages, dtype=torch.float32).to(device)
        # 创建数据集
        dataset = torch.utils.data.TensorDataset(
            states, actions, powers, log_probs_old, returns, advantages)
        ...
        for _ in range(epochs):
            for batch in data_loader:
                batch_states, batch_actions, batch_powers, batch_log_probs, batch_returns, batch_advantages = batch

                # Actor更新
                log_probs_new, entropy = self.actor.evaluate_actions(batch_states, batch_actions, batch_powers)
        ...
        if step % 50 == 0:
            torch.save(self.actor.state_dict(), 'model_actor_4.pth')
            torch.save(self.critic.state_dict(), 'model_critic_4.pth')
...
def step(state, action, power, user_cell_index, channel_gain_matrix, user_lambda1, user_lambda2, user_lambda3,
         traffic_list1,
         traffic_list2, traffic_list3, t,
         traffic1, traffic2, traffic3,rr):
    ...
    if len(beam_schedule) > 0:
        P = power[0:len(beam_schedule)]
    else:
        P = [250 / 8, 250 / 8, 250 / 8, 250 / 8, 250 / 8, 250 / 8, 250 / 8, 250 / 8]
    # P = np.clip(P, 10, 50)
    P = 0.8 * P_max / len(beam_schedule) * np.ones(len(beam_schedule)) + 0.2 * P_max * P
    user_index = []
    ...
    traffic_s1 = traffic_sums1 / np.sum(user_lambda1)
    traffic_s2 = traffic_sums2 / np.sum(user_lambda2)
    traffic_s3 = traffic_sums3 / np.sum(user_lambda3)
    ...
    return traffic1, traffic2, traffic3, next_state, reward, r1, r2, r3, r4, rr,done
...
agent.actor.load_state_dict(torch.load("model_actor_350.pth", map_location=torch.device('cpu')))
agent.actor.eval()
...
for episode in range(MAX_EPISODE):
    ...
    for T_s in range(0, 10):  # 2s
        ...
        while True:
            ...
            # 先做一个随机初始化扰动（均匀采样 [-epsilon, epsilon]）
            state_adv = state_tensor.clone().detach()
            agent.actor.eval()
            action, power, log_prob = agent.actor(state_tensor)
# 只在指定维度上加随机初始化扰动
            best_state_adv = state_tensor.clone()
            max_loss = -float('inf')

            for restart in range(num_restarts):
                # 随机初始化扰动
                state_adv = state_tensor.clone().detach()
                init_noise = torch.zeros_like(state_tensor)
                init_noise[0, 60*15:60*16] = state_tensor[0,60*15:60*16] * torch.empty(60).uniform_(-epsilon, epsilon).to(device)
                state_adv = state_adv + init_noise
                
                for tt in range(pgd_steps):
                    state_adv.requires_grad_(True)
                    log_prob_adv, _ = agent.actor.evaluate_actions(state_adv, action, power)
                    loss = -log_prob_adv.mean()
                    
                    agent.actor.zero_grad()
                    if state_adv.grad is not None:
                        state_adv.grad.zero_()
                    loss.backward()

                    grad = state_adv.grad.data
                    perturbation = alpha * grad.sign()
                    
                    mask = torch.zeros_like(state_adv)
                    mask[0, 60*15:60*16] = 1.0
                    state_adv = state_adv.detach() * (1 + perturbation * mask)

                    # 投影回 epsilon-ball
                    diff = torch.clamp(state_adv - state_tensor, min=-epsilon*state_tensor, max=epsilon*state_tensor)
                    state_adv = state_tensor + diff  
                # 保存当前 restart 的最大扰动
                final_loss = -agent.actor.evaluate_actions(state_adv, action, power)[0].item()
                
                
                if final_loss > max_loss:
                    max_loss = final_loss
                    
                    best_state_adv = state_adv.detach()
            # 最终输出多初始点中最优的对抗样本
            state_adv = best_state_adv
            ...
            with torch.no_grad():
                action_adv, power_adv, _ = agent.actor(state_adv)
                value_adv = agent.critic(state_adv).item()
    

            # 转 numpy，彻底断开梯度
            states.append(state.astype(np.float32))  # 如果 state 已经是 np.array
            actions.append(action_adv.cpu().numpy()[0])
            powers.append(power_adv.cpu().numpy()[0])
            #log_probs.append(float(log_prob)-float(log_prob_adv))  # 保证是纯 float
            P_a.append(float(log_prob)-float(log_prob_adv))
            traffic1, traffic2, traffic3, state, reward, reward1, reward2, reward3, reward4,rr, done = step(
                state,
                actions[-1],
                powers[-1],
                user_cell_index,
                ...
                t, traffic1, traffic2, traffic3,rr
            )
        ...
        agent.update(
            ...
            np.array(powers, dtype=np.float32),
    ...
    #np.savetxt('rr_FGSM_0.10_150.txt', P_aa)
    np.savetxt('P_aa_PGD_0.10_350.txt', P_aa)
    np.savetxt('reward_PGD_0.10_350.txt', Reward)
    np.savetxt('reward1_PGD_0.10_350.txt', Reward1)
    np.savetxt('reward2_PGD_0.10_350.txt', Reward2)
    np.savetxt('reward3_PGD_0.10_350.txt', Reward3)
    np.savetxt('reward4_PGD_0.10_350.txt', Reward4)
