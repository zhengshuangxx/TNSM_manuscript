...
def step(state, action, power, user_cell_index, channel_gain_matrix, user_lambda1, user_lambda2, user_lambda3,
    ...
    rr1=mask_h*(traffic_list1+traffic_list2+traffic_list3)+rr1
    traffic_sums1 = np.sum(traffic1, axis=1)
    traffic_sums2 = np.sum(traffic2, axis=1)
    traffic_sums3 = np.sum(traffic3, axis=1)
    ...
    #r1 = thr1#np.sum(traffic1[:, 0:29] * weights1[1:30])/np.sum(traffic1)
    #r2 = thr2#np.sum(traffic2[:, 0:9] * weights2[1:10])/np.sum(traffic2)
    #r3 = thr3#np.sum(traffic3[:, 0:99] * weights3[1:100])/np.sum(traffic3)
...
agent.actor.load_state_dict(torch.load("model_actor_442.pth", map_location=torch.device('cpu')))

# 加载 Critic 网络参数
agent.critic.load_state_dict(torch.load("model_critic_442.pth", map_location=torch.device('cpu')))
...
for episode in range(MAX_EPISODE):
    ...
    for T_s in range(0, 10):  # 2s
        ...
        Reward1_e.append(sum(reward1s) / tt_sum1)
        Reward2_e.append(sum(reward2s) / tt_sum2)
        Reward3_e.append(sum(reward3s) / tt_sum3)
    ...
    #np.savetxt('rr_3e-5_1.5.txt', rr)
    #np.savetxt('rr2_3e-5_1.5.txt', rr1)
    np.savetxt('reward_3e-5.txt', Reward)
    #np.savetxt('reward_3e-5.txt', Reward)
